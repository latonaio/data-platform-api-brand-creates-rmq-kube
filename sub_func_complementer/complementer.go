...
import (
	...
	"data-platform-api-brand-creates-rmq-kube/config"
	"encoding/json"
...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementBrand(input, l)
	if err != nil {
		return xerrors.Errorf("complement brand error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementBrand(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'Brand') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.Brand = nr.LatestNumber
	return &nr, nil
}
